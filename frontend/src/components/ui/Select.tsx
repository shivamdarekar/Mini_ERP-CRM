  ...
  ({ label, error, helperText, placeholder, className, id, children, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        {label && (
          <label htmlFor={selectId} className="text-xs font-bold text-slate-600 tracking-wider uppercase">
            {label}
          </label>
        )}
        <div className="relative flex items-center">
          <select
            ...
            id={selectId}
            className={cn(
              'w-full rounded-xl border bg-white pl-3.5 pr-10 py-2.5 text-sm text-slate-900 transition-all duration-200 shadow-sm appearance-none cursor-pointer',
